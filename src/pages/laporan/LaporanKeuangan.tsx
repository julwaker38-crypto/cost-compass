  ...
  <Layout>
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Pendapatan', value: formatCurrency(totalRevenue), icon: TrendingUp, color: 'text-green-600' },
          { label: 'Total Pengeluaran', value: formatCurrency(totalExpense), icon: TrendingDown, color: 'text-red-600' },
          { label: 'Laba Bersih', value: formatCurrency(totalProfit), icon: DollarSign, color: 'text-blue-600' },
          { label: 'Margin', value: `${margin}%`, icon: Percent, color: 'text-purple-600' },
        ].map(kpi => (
        ...
        <Card className="p-4">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={monthlyPL}>
              ...
              <YAxis fontSize={12} tickFormatter={v => `${(v / 1000000).toFixed(0)}jt`} />
              <Tooltip formatter={(v: number) => formatCurrency(v)} />
              <Line type="monotone" dataKey="profit" stroke="#10b981" strokeWidth={2} dot={{ r: 4 }} name="Laba" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>
